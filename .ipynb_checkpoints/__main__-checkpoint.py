...
class Player():
    ...
    def __init__(self):
        ...
        # self.x_vec, self.y_vec = Array([], dtype='float'), Array([], dtype='float')
    ...
    def append_point(self, x, y):
        self.x_vec = append(self.x_vec, (x,))
        self.y_vec = append(self.y_vec, (y,))
...
class Game():
    ...
    def check_for_intersection(self, x_vec, y_vec, x_last_vec, y_last_vec):
        ...
        for i in range(len(x_vec)):
            ...
            if all([  # largest small <= x <= smallest big
                max((min(x_vec[i:i+2]), min(x_last_vec))) <= x_intersection <= min((max(x_vec[i:i+2]), max(x_last_vec))),
                max((min(y_vec[i:i+2]), min(y_last_vec))) <= y_intersection <= min((max(y_vec[i:i+2]), max(y_last_vec)))
            ]):
                return True
